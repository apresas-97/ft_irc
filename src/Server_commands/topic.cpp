...
std::vector<t_message> Server::cmdTopic(t_message &message) {
    ...
    if (this->_channels.find(channelName) == this->_channels.end()) {
        replies.push_back(createReply(ERR_NOSUCHCHANNEL, ERR_NOSUCHCHANNEL_STR, {client->getNickname(), channelName}));
        return replies;
    }
    ...
    if (!channel.isUserInChannel(client->getNickname())) {
        replies.push_back(createReply(ERR_NOTONCHANNEL, ERR_NOTONCHANNEL_STR, {client->getNickname(), channelName}));
        return replies;
    }

    if (message.params.size() == 1) {
        if (channel.getTopic().empty()) {
            replies.push_back(createReply(RPL_NOTOPIC, RPL_NOTOPIC_STR, {client->getNickname(), channelName}));
        } else {
            replies.push_back(createReply(RPL_TOPIC, RPL_TOPIC_STR, {client->getNickname(), channelName, channel.getTopic()}));
        }
    } else {
        ...
        if (channel.getMode('t') && !channel.isUserOperator(client->getNickname())) {
            replies.push_back(createReply(ERR_CHANOPRIVSNEEDED, ERR_CHANOPRIVSNEEDED_STR, {client->getNickname(), channelName}));
            return replies;
        }
        ...
        topicMessage.prefix = client->getUserIdentifier();
